...
export class TMDBProvider extends BaseProvider {
  ...
  /**
   * Fetch director + top cast names for a single movie/TV item.
   * Uses the existing `director` column (currently never populated) —
   * no cast storage exists in the schema yet, so cast is returned
   * separately for the caller to decide whether to use it, rather than
   * silently dropped or fabricated into a column that doesn't exist.
   */
  async _fetchCredits(mediaType, id, token, attempt = 0) {
    try {
      const url = `${TMDB_BASE}/${mediaType}/${id}/credits?language=en-US`;
      const res = await fetch(url, { headers: this._getHeaders(token) });

      if (res.status === 429 && attempt < RETRY_ATTEMPTS) {
        await sleep(RETRY_DELAY_MS * (attempt + 1));
        return this._fetchCredits(mediaType, id, token, attempt + 1);
      }
      if (!res.ok) return null;
      ...
      const director =
        mediaType === "movie"
          ...
          : crew.find(
            ...
            crew.find((c) => c.job === "Executive Producer")?.name ||
            null;
      ...
      console.warn(
        `[TMDB] credits fetch failed for ${mediaType}/${id}:`,
        err.message,
      );
      return null;
    }
  ...
  /**
   * Fetch the genre ID -> name lookup once per run. TMDB's list/trending
   * endpoints only return genre_ids (numbers), never names — this is
   * what actually resolves them to real, displayable genre names instead
   * of storing raw IDs in the database.
   */
  async _fetchGenreMap(token) {
    const map = new Map();
    try {
      const [movieRes, tvRes] = await Promise.all([
        fetch(`${TMDB_BASE}/genre/movie/list?language=en-US`, {
          headers: this._getHeaders(token),
        }),
        fetch(`${TMDB_BASE}/genre/tv/list?language=en-US`, {
          headers: this._getHeaders(token),
        }),
      ]);
      if (movieRes.ok) {
      ...
      console.warn("[TMDB] genre list fetch failed:", err.message);
    }
    return map;
  }
  ...
  async fetch() {
    ...
    for (const entry of movieEntries) {
      ...
      entry.credits = await this._fetchCredits("movie", entry.item.id, token);
      trailerBudget--;
      await sleep(120); // gentle pacing between calls
    }

    for (const entry of tvEntries) {
      if (trailerBudget <= 0) break;
      entry.trailer = await this._fetchVideos("tv", entry.item.id, token);
      entry.credits = await this._fetchCredits("tv", entry.item.id, token);
      trailerBudget--;
      await sleep(120);
    }
    ...
    const genreMap = await this._fetchGenreMap(token);

    return { movies: movieEntries, tv: tvEntries, genreMap };
  }
  ...
  transform({ movies = [], tv = [], genreMap = new Map() }) {
    const items = [];

    for (const { item: m, signals, trailer, credits } of movies) {
      ...
      items.push({
        ...
        genre: (m.genre_ids || [])
          ...
          .filter(Boolean)
          .join(", "),
        // Uses the existing `director` column, previously never populated.
        director: credits?.director || null,
        // No `cast` column exists in the schema — stored in the existing
        // `metadata` jsonb column rather than fabricating a new column.
        metadata: credits?.cast ? { cast: credits.cast } : {},
    ...
    for (const { item: t, signals, trailer, credits } of tv) {
